...
export default function Page() {
    ...
    const fetchEntries = async (pageNum) => {
        try {
            const res = await axios.get(`http://192.168.0.122/accountList/${pageNum}`)
    ...
    const handleSearch = async (customFilter = filter) => {
        try {
            setIsSearching(true)
            const res = await axios.post('http://192.168.0.122/accountListSearch', {
                ...customFilter,
                page: 1,
                limit: limit
            })
    ...
    useEffect(() => {
        if (isSearching) {
            axios.post('http://192.168.0.122/accountListSearch', {
                ...filter,
                page: page,
                limit: limit
            }).then(res => {
    ...
    const handleDelete = async () => {
        ...
        try {
            for (const idx of selectedList) {
                await axios.put(
                    `http://192.168.0.122/accountDelete/${idx}`,{},{
                        headers: {
                            Authorization: token
                        }
                    }
    ...
    const handleEntryClick = async (entry_idx) => {
        try {
            const res = await axios.get(`http://192.168.0.122/accountDetail/${entry_idx}`)
