...
export default function ReceiptPaymentListPage() {
    ...
    const fetchData = async () => {
        await axios.get('http://192.168.0.122/receipt/list').then(res => {
            if (res.data.success) setReceiptList(res.data.list);
        });
        await axios.get('http://192.168.0.122/payment/list').then(res => {
    ...
    const handleDelete = async () => {
        ...
        for (const id of selectedItems) {
            await axios.put(`http://192.168.0.122/receipt/del/${id}`);
        }
        alert('삭제 완료');
        setSelectedReceipts([]);
        setSelectedPayments([]);
