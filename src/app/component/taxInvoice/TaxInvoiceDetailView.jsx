...
import '../../globals.css';
import TaxInvoiceDetailView from './TaxInvoiceDetailView';
...
const TaxInvoiceList = () => {
    ...
    const handlePreviewPdf = async (invoice_idx) => {
        try {
            const res = await axios.post('http://localhost:8080/taxInvoicePdf', null, {
                params: {
                    invoice_idx,
                    template_idx: 1,
                },
    ...
    return (
        <div className="flex">
            <div className="tax-invoice-list p-4 w-3/4">
                ...
                <table className="table-fixed w-full border">
                    ...
                    {invoiceList.map((item, index) => (
                        <tr key={item.invoice_idx} className="text-center border-t">
                            ...
                            <td className="p-2">
                                <button className="text-blue-600 underline" onClick={() => handlePreviewPdf(item.invoice_idx)}>미리보기</button>
                            </td>
            ...
            {/* 상세 사이드뷰 */}
            {selectedIdx && (
                <TaxInvoiceDetailView invoiceIdx={selectedIdx} onClose={() => setSelectedIdx(null)} />
            )}
