...
const SalesInsertPage = () => {
    ...
    const getProductList = async (searchText='') => {
        const {data} = await axios.post('http://192.168.0.122:8080/productNoption/list',{search:searchText});
        ...
        openModal({
            ...
            onConfirm: async() => {
                ...
                if(salesValid && productValid) {
                    try {
                        const {data} = await axios.post('http://192.168.0.122:8080/sales/insert', {
                            sales: salesForm,
