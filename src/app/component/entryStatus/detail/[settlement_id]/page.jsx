...
export default function EntryStatusDetailPage() {
    ...
    const handleDocumentCreate = async () => {
        ...
        try {
            const variableRes = await axios.post('http://192.168.0.122/settlement/preview', {
                template_idx: 12,
                variables: {
                    settlement_id: data.settlement_id,
                    settlement_day: data.settlement_day,
            ...
            if (res.data.success && res.data.document_idx) {
                ...
                setTimeout(async () => {
                    const detailRes = await axios.get(`http://192.168.0.122/settlement/detail/${data.settlement_id}`)
                    ...
                    {previewHtml ? (
                        <>
                            ...
                            <div style={{ marginTop: '30px' }}>
                                {docExists && data.file ? (
                                    <a
                                        href={`http://192.168.0.122/download/pdf/${data.document_idx}`}
