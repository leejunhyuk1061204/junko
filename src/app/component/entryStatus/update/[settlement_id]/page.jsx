...
export default function EntryStatusUpdatePage() {
    ...
    useEffect(() => {
        ...
        axios.get(`http://192.168.0.122/settlement/detail/${settlement_id}`)
            .then(res => {
                if (res.data.success) {
    ...
    useEffect(() => {
        if (form && form.entry_idx) {
            ...
            axios.get(`http://192.168.0.122/entry/settlement/previewAmount?entry_idx=${entryIdx}`)
                .then(res => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await axios.put('http://192.168.0.122/settlement/update', payload)
            if (res.data.success) {
                if (form.document_idx) {
                    const previewRes = await axios.post('http://192.168.0.122/settlement/preview', {
                        template_idx: 12,
                        variables: {
                            settlement_id: form.settlement_id,
                            settlement_day: form.settlement_day,
                    ...
                    if (previewRes.data.success) {
                        await axios.put('http://192.168.0.122/document/update', {
