...
export default function DocsModal({ productIdx, onClose }) {
    ...
    const fetchDocs = async () => {
        ...
        const res = await axios.get(`http://192.168.0.122/product/${productIdx}/docs`, {
    ...
    const handleUpload = async () => {
        ...
        await axios.post(`http://192.168.0.122/product/${productIdx}/docs`, formData, {
            headers: { Authorization: token }
        });
        setUploadFiles([]);
        await fetchDocs();
    ...
    const handleDelete = async (docId) => {
        const token = sessionStorage.getItem('token');
        await axios.put(`http://192.168.0.122/product/docs/${docId}/del`, null, {
            headers: { Authorization: token }
        });
        await fetchDocs();
    };
    ...
    const handleDownload = async (filename, oriFilename) => {
        const token = sessionStorage.getItem('token');
        const res = await axios.get(`http://192.168.0.122/product/docs/${filename}`, {
            headers: { Authorization: token },
            responseType: 'blob'
        });
                                ...
                                <div className="product-dropdown-wrap" style={{ position: 'relative' }}>
                                    ...
                                    {dropdownOpen[doc.file_idx] && (
                                        <div className="product-dropdown-content">
                                            <button
                                                onClick={() => {
                                                    window.open(`http://192.168.0.122/product/docs/${doc.new_filename}`, '_blank');
