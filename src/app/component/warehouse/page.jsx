            ...
            if(!selected.bool) {
                openModal({
                    ...
                    onConfirm: () => {
                        ...
                        setTimeout(async () => {
                            try {
                                const {data} = await axios.post('http://192.168.0.122/zone/update', {
                ...
                openModal({
                    ...
                    onConfirm: () => {
                        ...
                        setTimeout(async () => {
                            try {
                                const {data} = await axios.post('http://192.168.0.122/warehouse/update', {
        ...
        if(insertType){
            openModal({
                ...
                onConfirm: () => {
                    ...
                    setTimeout(async () => {
                        try {
                            const {data} = await axios.post('http://192.168.0.122/warehouse/insert', {
                                warehouse_name: warehouseForm.warehouse_name || '',
                                warehouse_address: warehouseForm.warehouse_address || '',
                                user_idx: warehouseForm.user_idx || 0
                            },{
            ...
            openModal({
                ...
                onConfirm: () => {
                    ...
                    setTimeout(async () => {
                        try {
                            const {data} = await axios.post('http://192.168.0.122/zone/insert', {
        ...
        if(selected.bool === true){
            openModal({
                ...
                onConfirm: () => {
                    ...
                    setTimeout(async () => {
                        try {
                            const {data} = await axios.get(`http://192.168.0.122/warehouse/del/${selected.warehouse.warehouse_idx}`);
                            console.log(data);
                            if (!data.success) {
                                openModal({
                                    svg: '❗',
            ...
            openModal({
                ...
                onConfirm: () => {
                    ...
                    setTimeout(async () => {
                        try {
                            const {data} = await axios.get(`http://192.168.0.122/zone/del/${selected.zone.zone_idx}`);
                            console.log(data);
                            if (!data.success) {
                                openModal({
                                    svg: '❗',
