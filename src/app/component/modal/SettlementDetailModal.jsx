...
export default function SettlementDetailModal({ data, onClose, showPdf, setShowPdf }) {
    ...
    const fetchFiles = async () => {
        try {
            const res = await axios.get(`http://localhost:8080/settlementFileList/${data.settlement_idx}`);
    ...
    const fetchLogs = async () => {
        try {
            const res = await axios.get(`http://localhost:8080/settlementLogs/${data.settlement_idx}`);
    ...
    const handleFileDownload = (fileIdx) => {
        window.open(`http://localhost:8080/settlementFileDown/${fileIdx}`);
    };
    ...
    const handleDownloadPdf = async () => {
        try {
            const res = await axios.post(`http://localhost:8080/settlementPdf`, null, {
                params: {
            ...
            link.href = `http://localhost:8080/static/pdf/${res.data.filename}`;
