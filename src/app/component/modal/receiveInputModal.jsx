...
const receiveInputModal = ({open,onClose,idx,status,getReceiveList}) => {
    ...
    const updateInfo = async () => {
        ...
        openModal({
            ...
            onConfirm: async() => {
                try {
                    console.log(info);
                    const {data} = await axios.post('http://192.168.0.122/receive/update', {receive_idx: idx, status: status, stockInfo:info, user_idx:selectedUser},{
                        headers: {
    ...
    const getReceiveProduct = async () => {
        const {data} = await axios.post('http://192.168.0.122/receiveProduct/list',{receive_idx:idx});
        console.log(idx);
        ...
        if(data.list.length > 0){
            const zone = await axios.post('http://192.168.0.122/zone/list',{warehouse_idx:data.list[0].warehouse_idx});
