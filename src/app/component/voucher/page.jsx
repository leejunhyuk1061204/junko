...
export default function VoucherListPage() {
    ...
    const fetchData = async () => {
        ...
        const res = await axios.get('http://192.168.0.122/voucher/list', {params})
        if (res.data.success) {
    ...
    const deleteVouchers = async () => {
        ...
        try {
            ...
            const deletePromises = checkedList.map((id) =>
                axios.put(`http://192.168.0.122/voucher/del/${id}`)
            )
    ...
    const handleStatusChange = async (entry_idx, newStatus) => {
        try {
            const res = await axios.put(`http://192.168.0.122/voucher/status/update/${entry_idx}`, {
                status: newStatus,
            });
