...
// 전표 데이터 불러오기
    useEffect(() => {
        if (!entry_idx) return
        axios.get(`http://192.168.0.122:8080/voucher/detail/${entry_idx}`)
            .then(res => {
                if (res.data.success) {
...
// approver_ids 기반 approvalLines 구성
    useEffect(() => {
        ...
        const fetchUsers = async () => {
            try {
                const res = await axios.post('http://192.168.0.122:8080/users/list', {})
                if (res.data.success) {
    ...
    useEffect(() => {
        ...
        const loadPreview = async () => {
            ...
            const url = previewMode === 'voucher'
                ? 'http://192.168.0.122:8080/voucher/preview'
                : 'http://192.168.0.122:8080/entry-detail/preview'
        ...
        try {
            ...
            if (res.data.success && res.data.document_idx) {
                ...
                if (pdfRes.data.success) {
                    const detailRes = await axios.get(`http://192.168.0.122:8080/voucher/detail/${voucher.entry_idx}`)
    ...
    const getDownloadLink = () => {
        ...
        if (previewMode === 'voucher') {
            return voucher.document_idx
                ? `http://192.168.0.122:8080/download/pdf/${voucher.document_idx}`
                : '#'
        } else {
            return voucher.entry_detail_document_idx
                ? `http://192.168.0.122:8080/download/pdf/${voucher.entry_detail_document_idx}`
                : '#'
        }
    }
