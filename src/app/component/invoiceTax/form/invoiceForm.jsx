...
export default function InvoiceFormPage({ isEdit = false, invoice_idx, onSubmitSuccess }) {
    ...
    useEffect(() => {
            ...
            .then(async res => {
                ...
                const filteredEntries = await Promise.all(
                    allEntries.map(async entry => {
                        const usedRes = await axios.get(`http://192.168.0.122/used/${entry.entry_idx}`)
        ...
        if (isEdit && invoice_idx) {
            axios.get(`http://192.168.0.122/invoice/detail/${invoice_idx}`)
                .then(res => {
    ...
    const createDocument = async (invoice_idx) => {
        try {
            ...
            const docRes = await axios.post('http://192.168.0.122/document/insert', payload)
            if (docRes.data.success && docRes.data.document_idx) {
                const document_idx = docRes.data.document_idx
                const pdfRes = await axios.post('http://192.168.0.122/document/pdf', { document_idx })
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (isEdit) {
                await axios.put('http://192.168.0.122/invoice/update', form)
                await createDocument(form.invoice_idx)
                alert('수정 완료')
            } else {
                const res = await axios.post('http://192.168.0.122/invoice/insert', form)
